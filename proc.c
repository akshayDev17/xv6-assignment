...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  // Initialize new fields added to the extended proc struct
...
exit(void)
{
  ...
  // pinfoTable[start_count].pid = curproc->pid;
  // pinfoTable[start_count].create_time = curproc->ctime;
  // pinfoTable[start_count].run_time = curproc->rtime;
  // pinfoTable[start_count].exit_time = curproc->etime;
  // start_count++;
...
getpinfo(){
  int i ;
  cprintf("\npid \t  CREATE_TIME \t RUNTIME \n");
  for(i = 0;i<start_count;i++){
    cprintf("%d \t   %d   \t %d \n",pinfoTable[i].pid, pinfoTable[i].create_time, pinfoTable[i].run_time);
  }
  return 0;
}
